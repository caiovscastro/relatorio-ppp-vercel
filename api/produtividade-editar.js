...
async function existsDuplicateExceptRow(sheets, { loja, matricula, dataBR, rowNumber }){
  ...
  for (let i=0;i<values.length;i++){
    ...
    if (sheetRowNumber === rowNumber) continue;
...
export default async function handler(req, res){
  ...
  try{
    ...
    if (!Number.isFinite(rowNumber) || rowNumber < 2){
      return bad(res, 400, "rowNumber inválido.");
    }
    ...
    const lojaResp = await sheets.spreadsheets.values.get({
      spreadsheetId,
      range: `${ABA}!C${rowNumber}:C${rowNumber}`,
      valueRenderOption: "FORMATTED_VALUE",
    });
    const lojaRow = lojaResp.data.values?.[0]?.[0] ?? "";

    if (!canViewAll(perfil)) {
      if (normKey(lojaRow) !== normKey(lojaSessao)){
        return bad(res, 403, "Perfil sem acesso.");
      }
    }

    const dup = await existsDuplicateExceptRow(sheets, {
      loja: lojaRow,
      matricula,
      dataBR: data,
      rowNumber
    ...
    await sheets.spreadsheets.values.update({
      spreadsheetId,
      range: `${ABA}!B${rowNumber}:H${rowNumber}`,
      valueInputOption: "USER_ENTERED",
      requestBody: {
        values: [[data, lojaRow, setor, matricula, nome, qtd, ""]],
      },
    });
    ...
    await sheets.spreadsheets.values.update({
      spreadsheetId,
      range: `${ABA}!I${rowNumber}:I${rowNumber}`,
      valueInputOption: "USER_ENTERED",
      requestBody: { values: [[usuario]] },
    });
    ...
    console.error("Erro produtividade-editar:", e);
    return bad(res, 500, "Erro ao editar PRODUTIVIDADE.", e?.message || String(e));
  }
}
